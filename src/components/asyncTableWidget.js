...
class AsyncTableWidget extends React.Component {
  ...
  _handleRowDelete(rowKeys) {
    ...
    for (let row of this.props.formData) {
      ...
      for (let key of rowKeys) {
        if (row[this.props.widgetData.keyField] === key) {
          flag = false;
        }
      }
      if (flag) {
        filteredRows = filteredRows.concat(row);
      }
    }
  ...
  render() {
    ...
    return (
      <BootstrapTable
        data={configs.formData}
        keyField={this.props.widgetData.keyField}
        cellEdit={cellEditProp}
        deleteRow={true}
        selectRow={selectRowProp}
        options={options}>
        {this.props.widgetData.tableCols.map(function(row, i) {
          if (row.customFieldType) {
            return (
              <TableHeaderColumn
                dataField={row.field}
