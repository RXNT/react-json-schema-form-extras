  ...
  if (action === "update") {
    return (cell, row, enumObject, rowIndex, formData, onChange) => {
      ...
      if (rowIndex != undefined) {
        let filterStatus = actionConfiguration.filterField
          ? actionConfiguration.filterField
          : false;
        if (filterStatus) {
          //If filter enabled on the table
          newFormData.map(function(value, index) {
            if (
              value[actionConfiguration.filterField] ===
              schema[actionConfiguration.filterField]["default"]
            ) {
